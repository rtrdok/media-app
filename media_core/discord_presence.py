...
def _local_cover(thumb: str | None) -> tuple[str, bytes, str] | None:
    """Read only generated library covers, never an arbitrary local path.

    Library items use /api/file?p=... URLs. Discord cannot reach localhost, so
    a valid generated cover is temporarily rehosted just like remote VK/Yandex
    covers. The strict directory check keeps the loopback player endpoint from
    becoming a file-upload primitive.
    """
    raw = (thumb or "").strip()
    parsed = urlparse(raw)
    ...
    if not values:
        return None
    try:
        path = Path(unquote(values[0])).resolve()
        # Both locations contain only images fetched/generated by Media App.
        # Do not loosen this to the whole cache or download directory.
        allowed = [
            (BASE_DIR / "file_cache" / "covers").resolve(),
            (BASE_DIR / "file_cache" / "thumbs").resolve(),
        ]
        is_generated_cover = False
        for folder in allowed:
            try:
                ...
                continue
        if not is_generated_cover:
            return None
        if not path.is_file() or path.stat().st_size > 4_500_000:
            return None
    ...
    if data.startswith(b"\x89PNG"):
        ...
        ext = "jpg"
    key = f"local:{path}:{path.stat().st_mtime_ns}:{path.stat().st_size}"
    return key, data, ext
...
def resolve_discord_large_image(thumb: str | None) -> str | None:
    ...
    cache_key = local[0] if local else src
    if not cache_key:
        return None
